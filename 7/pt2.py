...
class HandType(IntEnum):
    ...
    def get(cls, cards: list[Card]):
        if cards.count(Card("J")) == len(cards):
            return cls(7)
        
        mapped_cards: dict[Card: list[Card]] = dict()
        for card in cards:
            if card not in mapped_cards:
                mapped_cards[card] = []
            mapped_cards[card].append(card)

        replace_card = (0, Card("J"))
        if Card("J") in cards:
            replace_card = sorted([(len(mapped_cards[card]), card) for card in mapped_cards if card.card != "J"], reverse=True)[0]
        
        hand: list[int] = []
        for k in mapped_cards.values():
            if Card("J") in k:
                continue
            if replace_card[1] in k:
                hand.append(len(k) + len(mapped_cards[Card("J")]))
            else:
                hand.append(len(k))
        hand: list[int] = sorted(hand)
        match(len(hand)):
